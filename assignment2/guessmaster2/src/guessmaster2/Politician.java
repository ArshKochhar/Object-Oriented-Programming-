//Arsh Kochhar - 20104779

package guessmaster2;

public class Politician extends Person {
	
	
		private String party; //Instance Variable
		
		//initializing constructors 
		public Politician(String name, Date dateOfBirth, String gender, String party, double difficulty) {
			super(name, dateOfBirth, gender, difficulty);
			this.party = party;
		}
		
		//initializing constructors 
		public Politician(String name, String dateString, String gender, String party, double difficulty) {
			super(name, dateString, gender, difficulty);
			this.party = party;
		}
		
		
		//Copy-Constructor
		public Politician(Politician politician) {
			super(politician);
			this.party = politician.party;
		}
		
		
		//Accessors and Mutators for Politician party
		public String getParty() {
			return party;
		}
		public void setParty(String party) {
			this.party = party;
		}
		
		
		//Abstract Clone method
		public Politician clone() {
			return new Politician(this);
		}
		
		
		//To String function that calls parent class's toString function
		public String toString() {
			return super.toString()+"Party: "+getParty()+"\n";
		}
		
		
		//Abstract entityType method. Gives details about the type of derived entity, notice the '!'
		public String entityType() {
			return "This entity is a politician!";
		}

}
